...
var cache = {
  ...
  load: function () {
    ...
    if (data.scriptDir != scriptDir) return false;
    $.extend(this, data);
    return true;
  },
  save: function () {
    ...
    else window.name = JSON.stringify(this);
  },
...
var isOpera =
  ...
  !!window.opera ||
  navigator.userAgent.indexOf(" OPR/") >= 0; // Opera 8.0+
...
var isChrome =
  !!window.chrome && (!!window.chrome.webstore || !!window.chrome.csi); // Chrome 1+
...
(function () {
  ...
  } else setInitialSettings();

  function setInitialSettings() {
    // font size
    if (!isFrameCapable && cache.fontSize != 1)
      $("head").append(
        "<style>#right .area {font-size:" + cache.fontSize + "em}</style>"
      );
    // color theme
    if (cache.colorTheme) structure.setTheme(cache.colorTheme);
  }

  // Exit the script on sites which doesn't need the sidebar:
  if (forceNoScript || cache.forceNoScript) return;
  ...
  if (isFrameCapable) {
    if (isInsideFrame) {
      if (cache.fontSize != 1)
        $("head").append(
          "<style>body {font-size:" + cache.fontSize + "em}</style>"
        );
      normalizeParentURL = function () {
        postMessageToParent("normalizeURL", [
          $.extend({}, window.location),
          document.title,
          supportsHistory ? history.state : null,
          equivPath,
        ]);
          ...
          if (supportsHistory)
            history.replaceState(
              $.extend(history.state, {
                toc_clickItemTemp: cache.toc_clickItemTemp,
              }),
              null,
              null
            );
      ...
      normalizeParentURL();
      $(window).on("hashchange", normalizeParentURL);
      ...
      $(window).on("message onmessage", function (event) {
        ...
        switch (data[0]) {
          case "updateCache":
            if (typeof data[1] === "object") $.extend(cache, data[1]);
            else cache[data[1]] = data[2];
            break;
            ...
            try {
              ...
              if (history.replaceState)
                history.replaceState(
                  null,
                  null,
                  "?frame=" + encodeURI(relPath).replace(/#/g, "%23")
                );
            }
            document.title = data[2];
            if (structure.modifyTools) structure.modifyTools(relPath, data[4]);
            if (
              $("#left > div.toc li > span.selected a").attr("href") ==
              data[1].href
            )
              break;
            else if (data[3] && data[3].toc_clickItemTemp) {
              toc.deselect($("#left > div.toc"));
              $("#left > div.toc li > span")
                .eq(data[3].toc_clickItemTemp)
                .trigger("select");
            } else toc.preSelect($("#left > div.toc"), data[1]);
            break;
  ...
  if (isFrameCapable)
    $(document).ready(function () {
      if (window.sessionStorage)
        ...
        document.getElementById("frame").contentWindow.name =
          JSON.stringify(cache);
      structure.openSite(
        scriptDir + "/../" + (getUrlParameter("frame") || relPath)
      );
    });

  // Modify the site:
  structure.modify();
  if (!isFrameCapable) $(document).ready(features.add);
  toc.modify();
  index.modify();
  search.modify();
})();
...
function ctor_toc() {
  ...
  self.create = function (input) {
    // Create and add TOC items.
    ...
    for (var i = 0; i < input.length; i++) {
      ...
      if (path != "") {
        ...
        if (cache.deprecate_data[path]) el.className = "deprecated";
      } else var el = document.createElement("button");
      if (isIE8) el.innerHTML = text;
      else {
      ...
      if (cache.deprecate_data[path])
        li.title +=
          "\n\n" +
          T("Deprecated. New scripts should use {0} instead.").format(
            cache.deprecate_data[path]
          );
      if (subitems != undefined && subitems.length > 0) {
        li.className = "closed";
        li.innerHTML = span.outerHTML;
        li.innerHTML += self.create(subitems).outerHTML;
      } else li.innerHTML = span.outerHTML;
  ...
  self.preSelect = function ($toc, url) {
    ...
    var foundNoHashList = [];
    var url_href =
      url.href.slice(-1) == "/" ? url.href + "index.htm" : url.href;
    for (var i = 0; i < tocList.length; i++) {
      var href = tocList[i].firstChild.href;
      if (!href) continue;
      // Search for items matching the address:
      if (href == url_href) foundList.push($(tocList[i]));
    ...
    if (foundList.length) found = $(foundList).map($.fn.toArray);
    ...
    if (clicked.is(found)) el = clicked;
    else cache.set("toc_scrollPos", ""); // Force calculated scrolling.
    // If items are found:
    if (el) {
...
function ctor_index() {
  ...
  self.create = function (input, filter) {
    // Create and add the index links.
    var output = "";
    input.sort(function (a, b) {
      var textA = a[0].toLowerCase(),
        textB = b[0].toLowerCase();
      return textA.localeCompare(textB);
    });
    for (var i = 0, len = input.length; i < len; i++) {
      if (filter != -1 && input[i][2] != filter) continue;
      var a = document.createElement("a");
      a.href = workingDir + input[i][1];
      a.setAttribute("tabindex", "-1");
      if (isIE8) a.innerHTML = input[i][0];
      else {
  ...
  self.modify = function () {
    ...
    $indexSelect.on("change", function (e) {
      ...
      if (this.value == -1) $(this).addClass("empty");
    ...
    $indexInput.on("keyup input", function (e, noskip) {
      ...
      if (!noskip && input == prevInput) return;
      ...
      if (match.length) {
        ...
        if (!scrollTarget.length) {
          scrollTarget = indexListChildren.last();
        }
        scrollTarget[0].scrollIntoView(false);
        $this.attr("class", "match"); // 'items found'
      } else $this.attr("class", "mismatch"); // 'items not found'
    });
  ...
  self.findMatch = function (indexListChildren, input) {
    ...
    for (var i = 0; i < indexListChildren.length; i++) {
      var text = isIE8
        ? indexListChildren[i].innerText
        : indexListChildren[i].getAttribute("data-content");
  ...
  self.preSelect = function ($indexList, $indexInput) {
    // Apply stored settings.
    var clicked = $indexList.children().eq(cache.index_clickItem);
    $indexInput.val(cache.index_input);
    if (cache.index_scrollPos == null) $indexInput.trigger("keyup", true);
    else {
...
function ctor_search() {
  ...
  self.modify = function () {
    // Modify the elements of the search tab.

    if (
      !retrieveData(self.dataPath, "search_index", "SearchIndex", self.modify)
    )
      return;
    if (
      !retrieveData(self.dataPath, "search_files", "SearchFiles", self.modify)
    )
      return;
    if (
      !retrieveData(self.dataPath, "search_titles", "SearchTitles", self.modify)
    )
      return;
  ...
  self.preSelect = function ($searchList, $searchInput, $searchCheckBox) {
    // Apply stored settings.
    $searchInput.val(cache.search_input);
    if (cache.search_scrollPos == null) $searchInput.trigger("keyup", true);
    else {
      $searchList.html(cache.search_data);
      structure.addEventsForListBoxItems($searchList);
  ...
  self.convertToArray = function (SearchText) {
    // Convert text to array.
    // Normalize whitespace:
    SearchText = SearchText.toLowerCase().replace(/^ +| +$| +(?= )|\+/, "");
    if (SearchText == "") return "";
    // split and remove undefined or empty strings
    else
      return $(SearchText.split(" ")).filter(function () {
        return !!this;
      });
  };
  self.highlightWords = function (words) {
    ...
    } else content.removeHighlight();
  };
  self.scrollToMatch = function (direction) {
    var matches = $(isInsideFrame ? "body" : "#right .area").find(
      "span.highlight"
    );
  ...
  self.create = function (qry) {
    ...
    function file_has_all_words(file_index, words, start) {
      for (; start < words.length; ++start) {
        ...
        if (!iw || iw.indexOf(file_index) == -1) return false;
      }
      return true;
    }
    ...
    function get_results(w) {
      ...
      for (var i = 0; i < w.length && c < RESULT_LIMIT; ++i) {
        if (!file_has_all_words(w[i], qry, 1)) continue; // Skip files which don't have all the words.
        c++;
        var f = cache.search_files[w[i]];
        // data.files excludes '.htm' to save space, so add it back:
        if (f.indexOf("#") != -1) f = f.replace("#", ".htm#");
        else f = f + ".htm";
        // var ret_i = { u: location.href.replace(/\/docs\/.*/, "/docs/" + f),
        var ret_i = { u: f, t: cache.search_titles[w[i]] || f };
        ret.push(ret_i);
      }
      return ret;
    }
    ...
    function rank_results(aro, terms) {
      ...
      for (var i = 0; i < aro_ka.length; ++i)
        if (ukeys.indexOf(aro_ka[i]) == -1) ukeys.push(aro_ka[i]);
    ...
    function anyContains(str, arr) {
      // http://stackoverflow.com/a/5582640/883015
      for (var i = 0, len = arr.length; i < len; ++i) {
        if (str.indexOf(arr[i]) != -1) return 1;
      }
      return 0;
    }
    ...
    function decode_numbers(a) {
      ...
      var n = [];
      for (i = 0; i < a.length; i += 2) n.push(decode_number(a.substr(i, 2)));
      return n;
    }
    ...
    function index_partial(word) {
      if (word[0] == "+")
        // + prefix disables partial matching.
      ...
      for (var i = 0; i < files.length; ++i)
        if (unique.indexOf(files[i]) == -1) unique.push(files[i]);
...
function ctor_structure() {
  ...
  self.modify = function () {
    // Modify elements added via build.
    ...
    self.addShortcuts();
    self.addAnchorFlash();
    if (!isFrameCapable) self.setKeyboardFocus();
    if (supportsHistory) {
    ...
    $("#head")
      ...
      .each(function () {
        ...
        if (!attrDataTranslateValue || attrDataTranslateValue == 1) {
          if (elContent != "") $this.text(T(elContent));
    ...
    $main.find("li.color").on("click", function () {
      ...
      structure.setTheme(cache.colorTheme);
      if (isFrameCapable) postMessageToFrame("setTheme", [cache.colorTheme]);
    });
    ...
    self.modifyTools = function (relPath, equivPath) {
      ...
      $langList.find("li").each(function () {
        ...
        if (thisLink == null) $(this).hide(); // Hide language button
        else a.attr("href", thisLink + relPath);
      });
      ...
      $("li.edit > a").attr({
        href:
          T("https://github.com/Lexikos/AutoHotkey_L-Docs/edit/v1/docs/") +
          relPath,
        target: "_blank",
      });
      ...
      // 'Go back' button:
      registerEvent($chm.find("li.back"), "click", "", function () {
        history.back();
      });
      // 'Go forward' button:
      registerEvent($chm.find("li.forward"), "click", "", function () {
        history.forward();
      });
      // 'Zoom' button:
      registerEvent($chm.find("li.zoom"), "click", "", function () {
        cache.set("fontSize", cache.fontSize + 0.2);
        if (cache.fontSize > 1.4) cache.set("fontSize", 0.6);
        $("#frame")
          .contents()
          .find("body")
          .css("font-size", cache.fontSize + "em");
      });
      // 'Print' button:
      registerEvent($chm.find("li.print"), "click", "", function () {
    ...
    function processKeys($ListBox, keyCode) {
      ...
      switch (keyCode) {
        ...
        case 34: // Page Down
          var n = Math.floor(
            $ListBox.height() / $ListBox.children().first().outerHeight()
          );
          var direction = keyCode == 33 ? "prevAll" : "nextAll";
          if ($clicked[direction]().length < n)
            $ListBox
              .children()
              [keyCode == 33 ? "first" : "last"]()
              .click()
              .focus();
          else $clicked[direction]().eq(n).click();
          break;
    ...
    ListBox.on("keydown", function (e) {
      return processKeys($(this), e.which);
    });
    ...
    $("#left .checkbox input#highlightWords").on("change", function () {
      if (this.checked) {
        ...
        else search.highlightWords(cache.search_input);
      } else {
        cache.set("search_highlightWords", false);
        if (isFrameCapable) postMessageToFrame("highlightWords", []);
        else search.highlightWords("");
      }
    });

    $("#left .checkbox .up").on("click", function () {
      if (isFrameCapable) postMessageToFrame("scrollToMatch", ["prev"]);
      else search.scrollToMatch("prev");
    });

    $("#left .checkbox .down").on("click", function () {
      if (isFrameCapable) postMessageToFrame("scrollToMatch", ["next"]);
      else search.scrollToMatch("next");
    });
  ...
  self.displaySidebar = function (display) {
    ...
    if (display) {
      ...
      if (isPhone) $hTools.hide();
    } else {
      $headTabs.css(hide);
      $leftArea.css(hide);
      $dragbar.hide();
      if (isPhone) $hTools.show();
    }
  };
  ...
  self.showTab = function (pos) {
    ...
    $t.removeClass("selected").eq(pos).addClass("selected");
  ...
  self.updateQuickRef = function (relPath, h2s) {
    ...
    for (var i = 0; i < h2s.length; i++) {
      ...
      if (isIE8) el.innerHTML = h2.innerText;
      else {
  ...
  self.saveCacheBeforeLeaving = function () {
    $(window).on("beforeunload", function () {
      cache.set(
        "RightIsFocused",
        $(document.activeElement).closest("#right, #left > div.toc").length
      );
      cache.save();
    });
  };
  ...
  self.focusContent = function () {
    if (isFrameCapable) {
      ...
      else $("#frame").get(0).focus();
    } else $("#right").focus();
  };
  // Set keyboard focus at the right place after loading site:
  self.setKeyboardFocus = function () {
    $(window).on("load", function () {
      if (cache.RightIsFocused && !cache.clickTab) self.focusContent();
      else $("#left").find(".input input").focus();
    });
  };
  ...
  self.saveScrollPosOnScroll = function () {
    if (!history.state)
      // To have scrolling to top by default.
      history.replaceState(
        $.extend(history.state, { scrollTop: 0 }),
        null,
        null
      );
    $("#right").on("scroll", function () {
      history.replaceState(
        $.extend(history.state, { scrollTop: $(this)[0].scrollTop }),
        null,
        null
      );
    });
  ...
  self.addAnchorFlash = function () {
    $(window).on("load hashchange", function () {
      ...
      $(anchor).css(
        "backgroundColor",
        cache.colorTheme ? "#3e2f23" : "#ff9632"
      );
      setTimeout(function () {
        $(anchor)
          .css("backgroundColor", "")
  ...
  self.addEventsForListBoxItems = function (ListBox) {
    ...
    registerEvent(ListBox, "click", "> a", function () {
      ...
      if (!isScrolledIntoView($this, $parent)) {
        ...
        else $this[0].scrollIntoView(); // Move up
      }
      // Select the item:
...
function ctor_features() {
    ...
    for (var i = 0; i < tables.length; i++) {
      ...
      for (var j = 0; j < trs.length; j++) {
        ...
        for (var k = 0; k < tds.length; k++) {
          ...
          newTable +=
            "<td>" + first + "</td><td>" + td.innerHTML + "</td></tr>";
        }
        newTable += "</tbody>";
      }
      newTable += "</table>";
  ...
  self.modifyHeaders = function () {
    ...
    for (var i = 0; i < hs.length; i++) {
      ...
      if (!id) {
        // If head line doesn't have anchor...
        // As first child, try to use the id of its parent such as .methodShort:
        if (
          h.parentNode != self.content &&
          h.parentNode.getAttribute("id") &&
          !$(h).prev().length
        )
  ...
  self.gatherHeadingsForQuickRef = function () {
    ...
    postMessageToParent("updateQuickRef", [
      relPath.replace(window.location.hash, ""),
      list,
    ]);
  };
  ...
  self.modifyDeprecatedLinks = function () {
    ...
    for (var i = 0; i < as.length; i++) {
      ...
      if (cache.deprecate_data[path]) {
        ...
        a.title = T("Deprecated. New scripts should use {0} instead.").format(
          cache.deprecate_data[path]
        );
      }
    }
  ...
  self.modifyVersions = function () {
    ...
    for (var i = 0; i < spans.length; i++) {
      ...
      if ((m = /AHK_L (\d+)\+/.exec(text))) {
        ...
        title = m[3]
          ? T("Applies to AutoHotkey {0} and later").format(m[1])
          : "";
        if (!m[2]) m[1] = m[1] + ".00";
        if (m[1] < "v1.0.45.00") href = "ChangeLogHelp.htm#Older_Changes";
        else if (m[1] <= "v1.0.48.05") href = "ChangeLogHelp.htm#" + m[1];
        else href = "AHKL_ChangeLog.htm#" + m[1];
      } else continue;
  ...
  self.addCodeBoxButtons = function (pres) {
    for (var i = 0; i < pres.length; i++) {
      ...
      parent.className = "parent " + pre.className;
      if (isSyntax || isNoHighlight) pre.className = "origin no-highlight";
      else pre.className = "origin";
      ...
      if (isIE8) sel.innerHTML = "S";
      else sel.setAttribute("data-content", "S");
      ...
      if (!isSyntax && !isNoHighlight) {
        ...
        if (isIE8) dwn.innerHTML = "↓";
        else dwn.setAttribute("data-content", "↓");
      ...
      $(parent)
        .find("div.buttons > a.selectCode") // Select the code on click:
        .on("click", function () {
          var doc = document,
      ...
      $(parent)
        .find("div.buttons > a.downloadCode") // Download the code on click:
        .on("click", function (e) {
          ...
          var fileNameToSaveAs =
            pre.attr("filename") ||
            location.pathname.match(/([^\/]+)(?=\.\w+$)/)[0] + "-Script.ahk";
  ...
  self.addSyntaxColors = function (pres) {
    ...
    */
    if (isIE8)
      ...
      return;
    if (
      !retrieveData(index.dataPath, "index_data", "indexData", function () {
        self.addSyntaxColors(pres);
      })
    )
      return;
    ...
    for (var i = cache.index_data.length - 1; i >= 0; i--) {
      ...
      if (typeof type !== "undefined") {
        ...
        } else (syntax[type].single = syntax[type].single || []).push(entry);
    ...
    for (var i = 0; i < pres.length; i++) {
      ...
      if (pre.className.indexOf("no-highlight") != -1) continue;
      // Temporary remove elements which interfering with syntax detection:
      els.order.push("various");
      els.various = [];
      els.order.push("em");
      els.em = [];
      $(pre)
        ...
        .each(function () {
          if (this.tagName == "EM") {
            ...
            this.href &&
            this.getAttribute("href").substring(0, 4) != "http"
          )
      ...
      els.fun = [];
      innerHTML = innerHTML.replace(
        /^(\s*?static\s*?|\s*?)(\S*?)(?=\(.*?\)\s*(<(em|sct)><\/(em|sct)>\s*)*{)/gm,
        function (ASIS, PRE, DEFINITION) {
          if (DEFINITION.match(/^(while|if)$/i)) return ASIS;
      ...
      els.num = [];
      innerHTML = innerHTML.replace(
        /\b((0(x|X)[0-9a-fA-F]*)|(([0-9]+\.?[0-9]*)|(\.[0-9]+))((e|E)(\+|-)?[0-9]+)?)\b/gm,
        function (_, NUMBER) {
      ...
      els.order.push("cont1");
      els.cont1 = [];
      innerHTML = innerHTML.replace(
        /('|")([<\/em>\s]*?^\s*\([\s\S]*?^\s*\).*?)\1/gm,
        function (_, QUOTE, SECTION) {
      ...
      els.order.push("cont2");
      els.cont2 = [];
      innerHTML = innerHTML.replace(
        /(^\s*:.*?:.*?::)([<\/em>\s]*?^\s*\([\s\S]*?^\s*\).*?)/gm,
        function (_, PRE, SECTION) {
          out = processStrParam(SECTION);
          els.cont2.push(out);
      ...
      els.biv = [];
      innerHTML = innerHTML.replace(
        new RegExp("\\b(" + syntax[1].single.join("|") + ")\\b", "gi"),
        function (_, BIV) {
          out = wrap(BIV, "biv", 1);
          els.biv.push(out);
          return "<biv></biv>";
      ...
      els.order.push("dec_cls");
      els.dec_cls = [];
      innerHTML = innerHTML.replace(
        /(^\s*)(class)(\s+\S+\s+)(extends)\b/gim,
        function (_, PRE, CLASS, INPUT, EXTENDS) {
      ...
      innerHTML = innerHTML.replace(
        new RegExp(
          "(" +
            syntax[0].single.join("|") +
            ")\\b($|[\\s,])(.*?)(?=<(?:em|sct)></(?:em|sct)>|$)",
          "gim"
        ),
        function (_, DIR, SEP, PARAMS) {
          ...
          if (PARAMS.length > types.length)
            // For the last param of any directive.
            PARAMS.push(PARAMS.splice(types.length - 1).join(","));
          // Iterate params and recompose them:
          for (n in PARAMS) {
            // Restore (...), {...} and [...] previously excluded:
            PARAMS[n] = PARAMS[n].replace(
              /<sub (\d+)><\/sub>/g,
              function (_, index) {
                return sub[index];
              }
            );
            if (PARAMS[n].match(/^\s*%\s/))
              // Skip forced expression parameter:
              continue;
            if (types[n] == "S")
              // string
      ...
      els.cls = [];
      innerHTML = innerHTML.replace(
        new RegExp("\\b(" + syntax[6].single.join("|") + ")\\b", "gi"),
        function (_, CLS) {
          out = wrap(CLS, "cls", 6);
          els.cls.push(out);
          return "<cls></cls>";
        ...
        function (ASIS, IF, INPUT, BETWEEN, VAL, CFS, SEP, PARAMS) {
          if (IF) {
            if (INPUT) {
              var cfs =
                cache.index_data[
                  syntax[3].dict[(IF + " ... " + BETWEEN).toLowerCase()]
                ];
              if (cfs)
                out =
                  wrap(IF, "cfs", cfs[1]) +
                  INPUT +
                  wrap(BETWEEN, "cfs", cfs[1]) +
                  (cfs[3][1] == "S" ? processStrParam(VAL) : VAL);
              else out = ASIS;
            }
          } else {
      ...
      // hotstrings:
      els.order.push("hotstr");
      els.hotstr = [];
      innerHTML = innerHTML.replace(
        /^(\s*)(:.*?:)(.*?)(::)(.*)/gm,
        function (_, PRE, HOTSTR1, ABBR, HOTSTR2, REPL) {
          out =
            PRE +
            wrap(HOTSTR1, "lab", null) +
            wrap(ABBR, "str", null) +
            wrap(HOTSTR2, "lab", null) +
            (HOTSTR1.match(/x/i) ? REPL : wrap(REPL, "str", null));
          els.hotstr.push(out);
          return "<hotstr></hotstr>";
        }
      );
      // hotkeys:
      els.order.push("hotkey");
      els.hotkey = [];
      innerHTML = innerHTML.replace(
        /^(\s*)((([#!^+*~$]|&lt;|&gt;)*(\S+)( up)?|~?(\S+) &amp; ~?(\S+)( up)?)::)/gim,
        function (_, PRE, HOTKEY) {
      ...
      els.lab = [];
      innerHTML = innerHTML.replace(
        /^(\s*)([^\s{(]+?:)(?=\s*(<(em|sct)><\/(em|sct)>|$))/gm,
        function (_, PRE, LABEL) {
      ...
      for (var k = els.order.length - 1; k >= 0; k--) {
        $(pre)
          ...
          .each(function (index) {
            ...
            else this.outerHTML = els[els.order[k]][index];
          });
      }
    }
      ...
      if (TypeOrLink != null) {
        ...
        if (typeof TypeOrLink == "number")
          a.href =
            scriptDir +
            "/../" +
            cache.index_data[syntax[TypeOrLink].dict[match.toLowerCase()]][1];
        else a.href = scriptDir + "/../" + TypeOrLink;
        a.innerHTML = match;
        span.appendChild(a);
      } else span.innerHTML = match;
      return span.outerHTML;
    }
    function processStrParam(param) {
      if (param.match(/^\s*(\+|-|\^)?\s*<num><\/num>\s*$/))
        // skip number
        return param;
  ...
  self.addFooter = function () {
    ...
    div.innerHTML =
      ...
      ' - LIC: <a href="' +
      scriptDir +
      '/../license.htm" class="no-ext">GNU GPLv2</a>';
...
function getWorkingDir() {
  ...
  for (i = 0; i < pathArray.length - 1; i++) wDir += pathArray[i] + "/";
  return wDir;
}
...
function postMessageToFrame(id, param) {
  ...
  document
    .getElementById("frame")
    .contentWindow.postMessage(JSON.stringify(param), "*");
}
...
function loadJQuery() {
  ...
  /*Search highlighting*/ jQuery.fn.highlight = function (c) {
    function e(b, c) {
      var d = 0;
      if (3 == b.nodeType) {
        var a = b.data.toUpperCase().indexOf(c),
          a =
            a -
            (b.data.substr(0, a).toUpperCase().length -
              b.data.substr(0, a).length);
        if (0 <= a) {
  ...
  $.queueFunc = {
    // debuggable.com (run-intense-js-without-freezing-the-browser)
    _timer: null,
    _queue: [],
...
function addPrototypeMethods() {
  ...
  if (!Array.prototype.indexOf) {
    Array.prototype.indexOf = function (searchElement, fromIndex) {
      ...
      if (str.indexOf("," + searchElement + ",") !== -1) return 0;
      else return -1;
    };
  }
